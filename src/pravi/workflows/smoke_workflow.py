"""Slice 0 smoke workflow: create worktree → optional command → cleanup.

Kept around as a no-LLM harness for verifying the Temporal + worktree plumbing.
`pravi ticket run --fake` calls this. The real per-ticket lifecycle lives in
[`feature_workflow.py`](feature_workflow.py).
"""
